...
final class SmokeMatrixRunner
{
    ...
    {
        return array_map(static fn(int $number): string => sprintf('BT-SMOKE-%03d', $number), range(1, 20));
    }
        ...
        try {
            switch ($caseId) {
                ...
                case 'BT-SMOKE-012':
                    ...
                    break;
                case 'BT-SMOKE-013':
                    $details = $this->pageTreeService->getPageDetails((int)$fixture['pages']['batchTests'], 1, (int)$fixture['pages']['root']);
                    $longPreview = false;
     ...
     */
    private function previewOnly(BatchSelection $selection): array
    {
        $plan = $this->preflightService->buildPlan($selection);
        $preview = $this->previewService->buildPreview($plan, 'DE', 'EN-US');
        $jobUid = $this->jobLogger->createJobFromPlan($preview['plan'], 'previewed');
     ...
     */
    private function previewAndWrite(BatchSelection $selection): array
    {
        [$plan, $jobUid] = $this->previewOnly($selection);
     ...
     */
    private function selection(array $fixture, TranslationMode $mode = TranslationMode::TranslateMissingOnly, array $pages = [], array $subtrees = [], array $elements = [], array $excludedPages = [], array $excludedElements = []): BatchSelection
    {
    ...
    private function assertSourceContains(string $needle, string $message): array
    {
        ...
        $found = false;
        foreach (['Resources/Private/Templates/BatchTranslation/Index.html', 'Resources/Public/Javascript/backend-scroll.js'] as $file) {
            $path = $basePath . '/' . $file;
    ...
    private function targetPageUid(int $sourceUid): int
    {
    ...
    private function targetContentUid(int $sourceUid): int
    {
